  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (fulfillmentMethod === "delivery") {
      ...
      if (!houseNo || !street || !barangay || !city || !zipcode) {
        setSubmitError("Please complete all required address fields (House No, Street, Barangay, City, and Zip Code).");
        return;
      }
      address = [
        houseNo,
    ...
    if (email.endsWith('.co') && !email.endsWith('.co.uk') && !email.endsWith('.co.za')) {
      setSubmitError("Did you mean '.com' instead of '.co'? Please check your email address.");
      return;
    }
    ...
    // Validate file upload more carefully
    if (!receiptFile || !(receiptFile instanceof File) || !receiptFile.name || receiptFile.size === 0) {
      setSubmitError("Please upload your GCash receipt screenshot.");
      return;
    }

    if (!receiptFile.type.startsWith("image/")) {
      setSubmitError("Receipt screenshot must be an image file (PNG, JPG, etc.).");
      return;
    }
